...
namespace GPSControlSMS.Tests
{
    ...
    public class SenderViewModelTests
    {
        ...
        public async Task InitializeAsync_ShouldLoadUnitsAndDefaultValues()
        {
            ...
            // Assert
            Assert.That(units.First() == _viewModel.Unit);
            Assert.That(units.First().Device == _viewModel.SelectedDevice);
            Assert.That(units.First().Name == _viewModel.SelectedUnit);
            CollectionAssert.AreEqual(units.Select(u => u.Name).ToList(), _viewModel.Names);
        }
        ...
        public async Task AddUnit_ShouldAddNewUnit_WhenValid()
        {
            ...
            // Assert
            Assert.That("NewUnit" == _viewModel.SelectedUnit);
        ...
        public void OnUnitModelChanged_ShouldUpdateUnitAndSelectedDevice()
        {
            ...
            // Assert
            Assert.That(unit == _viewModel.Unit);
            Assert.That(unit.Device == _viewModel.SelectedDevice);
        }
        ...
        public void LoadCommands_ShouldUpdateCommandList()
        {
            ...
            // Assert
            Assert.That(commands.Count == _viewModel.CommandList.Count());
        }
    }
}